...
@test "check if -h is implemented" {
    run ./ls -h ${TEST_DIR}
    if [[ "$output" == *"invalid option -- 'h'"* ]]; then
        [ 1 -eq 0 ]  # Force test to fail if error message is present
    fi
}

@test "check if symbolic link is implemented" {
    # Create a symbolic link
    ln -s "${TEST_DIR}/a" "${TEST_DIR}/a.link"
    ...
    # Check if the output contains the symbolic link information
    if [[ "$output" != *"a.link -> a"* ]]; then
        echo "Symbolic link not properly handled in output:"
        echo "$output"
        return 1
    fi
}

@test "check if --hack is implemented" {
    run ./ls --hack ${TEST_DIR}
    if [[ "$output" == *"unrecognized option '--hack'"* ]]; then
        [ 1 -eq 0 ]  # Force test to fail if error message is present
    fi
}
